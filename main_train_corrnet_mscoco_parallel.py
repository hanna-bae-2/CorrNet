...
def main(dataset_folder, description_size, temperature, epochs, batch_size, learning_rate, weight_decay, training_mode,
    ...
    loader_train = DataLoader(data_train, batch_size=batch_size, shuffle=True, num_workers=1, drop_last=True, sampler=train_sampler)
    val_sampler = DistributedSampler(dataset=data_val)
    loader_val = DataLoader(data_val, batch_size=batch_size, shuffle=False, num_workers=1, drop_last=False, sampler=val_sampler)
    ...
    cornet = torch.nn.parallel.DistributedDataParallel(cornet, device_dis=[local_rank], output_device=local_rank)
    cornet.train()
