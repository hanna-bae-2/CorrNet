...
def init_for_distributed(rank, opts):
    ...
    # convert pring fn if rank is zero 
...
def main(dataset_folder, description_size, temperature, epochs, batch_size, learning_rate, weight_decay, training_mode,
         epochs_ft, learning_rate_ft, weight_decay_ft, device, experiment_folder, pre_trained, gradient_accumulation, rank, world_size):
    ...
    # Load data loader
    # add the DDp Dataset Example 
    train_sampler = DistributedSampler(dataset = data_train, shuffle=True) 
    test_sampler = DistributedSampler(dataset=data_val, shuffle=False )

    loader_train = DataLoader(dataset=data_train, batch_size=batch_size/world_size, shuffle=False, num_workers=4/world_size,
                              sampler=train_sampler, pin_memory=True)
    loader_val = DataLoader(dataset=data_val, batch_size=batch_size/world_size, shuffle=False, num_workers=4/world_size,
    ...
    cornet = CorrNet(feature_dim=description_size).to(device)
    cornet = DDP(module=cornet, device_ids = [rank])
...
if __name__ == '__main__':
    ...
    try:
        main(df, ds, t, e, bs, lr, wd, tm, ef, lrf, wdf, cd, exp_f, p, ga, args.rank, args.world_size)
